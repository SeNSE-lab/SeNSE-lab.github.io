...
$lang='en';
$use_auth=true;
$readonly_users=array('userchik');
$show_hidden_files=true;
$use_highlightjs=true;
$highlightjs_style='solarized-dark';
$edit_files=true;
$send_mail=false;
$toMailId="";
...
$auth_users=array('odmin'=>'6627415e807ee33c7302917216e7da68','userchik'=>'2c9d9639de5986ca053cc0bc1de6d5bc');
if(empty($auth_users)) {
    $use_auth=false;
}
...
?>
       <div class="path login-form">
        ...
        echo fm_t('Username', $lang);
?></label><input type="text" id="fm_usr" name="fm_usr" value="" placeholder="<?php
        echo fm_t('Username', $lang);
?>" required><br>
                <label for="fm_pwd"><?php
        echo fm_t('Password', $lang);
?></label><input type="password" id="fm_pwd" name="fm_pwd" value="" placeholder="<?php
        echo fm_t('Password', $lang);
?>" required><br>
...
?>">
            ...
            <br>
            <p>
                <button type="submit" class="btn"><i class="fa fa-check-circle"></i> <?php
    echo fm_t('Upload');
?></button> &nbsp;
                <b><a href="?p=<?php
...
?></b></p>
            <p class="break-word"><?php
    echo fm_t('Source folder:');
?> <?php
    echo fm_convert_win(FM_ROOT_PATH . '/' . FM_PATH);
?><br>
                <label for="inp_copy_to"><?php
    echo fm_t('Destination folder:');
?></label>
                <?php
    echo FM_ROOT_PATH;
...
?>">
            ...
            <p><label><input type="checkbox" name="move" value="1"> <?php
    echo fm_t('Move');
?></label></p>
            <p>
                <button type="submit" class="btn"><i class="fa fa-check-circle"></i> <?php
...
?>&amp;copy=<?php
    echo urlencode($copy);
?>&amp;finish=1"><i class="fa fa-check-circle"></i> <?php
    echo fm_t('Copy');
?></a></b> &nbsp;
            <b><a href="?p=<?php
    echo urlencode(FM_PATH);
?>&amp;copy=<?php
    echo urlencode($copy);
?>&amp;finish=1&amp;move=1"><i class="fa fa-check-circle"></i> <?php
    echo fm_t('Move');
?></a></b> &nbsp;
            <b><a href="?p=<?php
    echo urlencode(FM_PATH);
...
?>&amp;unzip=<?php
        echo urlencode($file);
?>&amp;tofolder=1" title="<?php
        echo fm_t('UnZip to');
?> <?php
        echo fm_enc($zip_name);
?>"><i class="fa fa-check-circle"></i>
                    <?php
        echo fm_t('UnZip to folder');
?></a></b> &nbsp;
                <?php
    }
...
?>
           <b><a href="?p=<?php
    echo urlencode(FM_PATH);
?>"><i class="fa fa-chevron-circle-left"></i> <?php
    echo fm_t('Back');
?></a></b>
        </p>
        <?php
...
<?php
if(!FM_READONLY):
?><th style="width:3%"><label><input type="checkbox" title="<?php
    echo fm_t('Invert selection');
?>" onclick="checkbox_toggle()"></label></th><?php
endif;
?>
<th><?php
echo fm_t('Name');
?></th><th style="width:10%"><?php
echo fm_t('Size');
?></th>
...
?>
<a title="<?php
    echo fm_t('Direct link');
?>" href="<?php
    echo FM_ROOT_URL . (FM_PATH!='' ? '/' . FM_PATH : '') . '/' . $f . '/';
...
?></div></td>
<td><span title="<?php
    printf(fm_t('%s bytes'), $filesize_raw);
?>"><?php
    echo $filesize;
?></span></td>
<td><?php
...
<?php
    echo fm_t('Full size:');
?> <span title="<?php
    printf(fm_t('%s bytes'), $all_files_size);
?>"><?php
...
if(!FM_READONLY):
?>
<p class="path footer-links"><a href="#/select-all" class="group-btn" onclick="select_all();return false;"><i class="fa fa-check-square"></i> <?php
    echo fm_t('Select all');
?></a> &nbsp;
<a href="#/unselect-all" class="group-btn" onclick="unselect_all();return false;"><i class="fa fa-window-close"></i> <?php
    echo fm_t('Unselect all');
?></a> &nbsp;
<a href="#/invert-all" class="group-btn" onclick="invert_all();return false;"><i class="fa fa-th-list"></i> <?php
    echo fm_t('Invert selection');
?></a> &nbsp;
<input type="submit" class="hidden" name="delete" id="a-delete" value="<?php
    echo fm_t('Delete');
?>" onclick="return confirm('<?php
    echo fm_t('Delete selected files and folders?');
?>')">
<a href="javascript:document.getElementById('a-delete').click();" class="group-btn"><i class="fa fa-trash"></i> <?php
    echo fm_t('Delete');
?> </a> &nbsp;
<input type="submit" class="hidden" name="zip" id="a-zip" value="<?php
    echo fm_t('Zip');
?>" onclick="return confirm('<?php
    echo fm_t('Create archive?');
?>')">
<a href="javascript:document.getElementById('a-zip').click();" class="group-btn"><i class="fa fa-file-archive-o"></i> <?php
    echo fm_t('Zip');
?> </a> &nbsp;
<input type="submit" class="hidden" name="copy" id="a-copy" value="<?php
    echo fm_t('Copy');
?>">
<a href="javascript:document.getElementById('a-copy').click();" class="group-btn"><i class="fa fa-files-o"></i> <?php
    echo fm_t('Copy');
?> </a>
<a href="#" target="_blank" class="float-right">Tiny File Manager</a></p>
<?php
endif;
?>
</form>
...
?>
       <a title="<?php
        echo fm_t('Search');
?>" href="javascript:showSearch('<?php
        echo urlencode(FM_PATH);
?>')"><i class="fa fa-search"></i></a>
        <a title="<?php
        echo fm_t('Upload files');
?>" href="?p=<?php
        echo urlencode(FM_PATH);
?>&amp;upload"><i class="fa fa-cloud-upload" aria-hidden="true"></i></a>
        <a title="<?php
        echo fm_t('New folder');
?>" href="#createNewItem" ><i class="fa fa-plus-square"></i></a>
        <?php
    endif;
?>
       <?php
...
?><a title="<?php
        echo fm_t('Logout');
?>" href="?logout=1"><i class="fa fa-sign-out" aria-hidden="true"></i></a><?php
    endif;
?>
       </div>
</div>
...
function rename(p,f){var n=prompt('<?php
    echo fm_t('New name');
?>',f);if(n!==null&&n!==''&&n!=f){window.location.search='p='+encodeURIComponent(p)+'&ren='+encodeURIComponent(f)+'&to='+encodeURIComponent(n);}}
